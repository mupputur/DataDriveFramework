...
def get_chrome_driver_download_url(version):

    # major_version = ch_version
    # print(major_version)
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Failed to get ChromeDriver version for Chrome {version}")


def download_and_extract_chromedriver(url, extract_path=".dependencies//chromedriver"):
    try:
        ...
        if response.status_code == 200:
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_ref:
                ...
                print(f"Extracted ChromeDriver to: {extract_path}")

            os.remove(zip_path)

            print(f"ChromeDriver is ready at {extract_path}\\chromedriver.exe")
            return True
